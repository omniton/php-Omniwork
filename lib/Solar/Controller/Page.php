...
abstract class Solar_Controller_Page extends Solar_Base
{
     ...
     *
     */
    protected function _render($layout = null, $view = null, $adapter = array())
    {
        // if no view and no layout, there's nothing to render
        if (! $this->_view && ! $this->_layout && ! $layout && ! $view) {
            $this->_setContentType();
            return;
        }

        $this->_setViewObject($adapter);
        $this->_preRender();
        $this->_view_object->assign($this);

        // save current layout & view and replace by custom
        if ($layout || $view) {
            $layoutBak     = $this->_layout;
            $this->_layout = $layout;
            $viewBak       = $this->_view;
            $this->_view   = $view;
        }
        ...
        // restore current layout & view
        if ($layout || $view) {
            $this->_layout = $layoutBak;
            $this->_view   = $viewBak;
        }
     ...
     *
     */
    protected function _setViewObject($adapter)
    {
        // set up a view object, its template paths, and its helper stacks
        $adapter = empty($adapter) ? array_value($this->_config, 'view') : $adapter;
        $this->_view_object = Solar::dependency('Solar_View', $adapter);
    ...
    protected function _addViewTemplates()
    {
        ...
        $classes = array_reverse(Solar_Class::parents($this, true));
        ...
        foreach ($classes as $val) {
            $stack[] = str_replace('_', DIRECTORY_SEPARATOR, $val) . DIRECTORY_SEPARATOR . 'View';
        }
    ...
    protected function _setLayoutTemplates()
    {
        ...
        // convert underscores to slashes, and add /Layout
        foreach ($classes as $val) {
            $stack[] = str_replace('_', DIRECTORY_SEPARATOR, $val) . DIRECTORY_SEPARATOR . 'Layout';
        }
