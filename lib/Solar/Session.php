...
class Solar_Session extends Solar_Base
{
    ...
    protected function _postConstruct()
    {
        ...
        //if (! self::$_handler) {
        //    self::$_handler = Solar::factory($this->_config['handler']);
        //}
        if (! self::$_handler) {
    ...
    public function popFlash($key, $val = null)
    {
        $this->load();

        if (array_key_exists($key, $this->_flash)) {
            $array = (array) $this->_flash[$key];
            $val = array_pop($array);
            if (empty($array)) {
                unset($this->_flash[$key]);
            } else {
    ...
    public function shiftFlash($key, $val = null)
    {
        $this->load();
    ...
    public function raiseMessage($message, $messageType = 'error')
    {
        $this->setFlash('message', $message);
        $this->setFlash('messageType', $messageType);
        return;
    }
